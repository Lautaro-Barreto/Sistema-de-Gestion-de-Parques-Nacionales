...
/*         IF NOT EXISTS (SELECT * FROM sys.objects WHERE type = 'U' AND name = 'Staging_Raw_CSV')
        ...
        IF EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Area_Infraestructura' AND TABLE_NAME='Staging_Raw_CSV')
            DROP TABLE Area_Infraestructura.#Staging_Raw_CSV;

        CREATE TABLE Area_Infraestructura.#Staging_Raw_CSV (
        ...
        SET @sql = N'
        BULK INSERT Area_Infraestructura.#Staging_Raw_CSV
        FROM ''' + @RutaArchivo + '''
        WITH (
        ...
        FROM Area_Infraestructura.#Staging_Raw_CSV
    ...
    END CATCH
    DROP TABLE IF EXISTS Area_Infraestructura.#Staging_Raw_CSV; -- Aseguramos eliminar la tabla raw si quedó por algún error
END